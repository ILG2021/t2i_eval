...
def attention_function(q, k, v, heads, mask=None):
    b, s, dim = q.shape
    dim_head = dim // heads
    q = q.view(b, s, heads, dim_head).transpose(1, 2)
    k = k.view(b, s, heads, dim_head).transpose(1, 2)
    v = v.view(b, s, heads, dim_head).transpose(1, 2)
...
def rope(pos: torch.Tensor, dim: int, theta: int) -> torch.Tensor:
    assert dim % 2 == 0
    device = pos.device
    # Simplified for CUDA/CPU
    scale = torch.arange(0, dim, 2, dtype=torch.float64, device=device) / dim
    omega = 1.0 / (theta**scale)
    out = torch.einsum("...n,d->...nd", pos.to(device).to(torch.float64), omega)
...
class ErnieImageModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, x, timesteps, context, **kwargs):
        ...
        image_ids[:, :, 1] = image_ids[:, :, 1] + torch.linspace(0, Hp - 1, steps=Hp, device=device, dtype=torch.float32).unsqueeze(1)
        image_ids[:, :, 2] = image_ids[:, :, 2] + torch.linspace(0, Wp - 1, steps=Wp, device=device, dtype=torch.float32).unsqueeze(0)
